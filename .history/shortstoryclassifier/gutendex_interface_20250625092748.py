...
df = df.dropna(subset=["shorttitle", "author"])
df = df.drop_duplicates(subset=["shorttitle", "author"])
books_to_fetch = list(df[["docid", "shorttitle", "author"]].itertuples(index=False, name=None))

results_metadata = []
success_count = 0

def flip_author(name):
    if pd.isna(name):
        return ""
...
df["search_author"] = df["author"].apply(flip_author)
...
for docid, title, author in tqdm(books_to_fetch):
    ...
    match = process.extractOne(title, [b["shorttitle"] for b in candidates], scorer=fuzz.token_sort_ratio)
    if match and match[1] >= 75:
        matched_title = match[0]
        chosen = next(b for b in candidates if b["shorttitle"] == matched_title)
