...
def clean_genres(val):
    ...
    try:
        ...
        if isinstance(genres, str):  # in case literal_eval failed silently
            genres = [genres]
    except:
        genres = [val.strip()]
...
df = df.drop(columns=['access_rights', 'part_of_journal'], errors='ignore') # would take 4 hours to fetch
