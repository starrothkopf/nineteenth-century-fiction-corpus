...
def is_british_irish_loose(row):
    bookshelves = row.get("bookshelves", "").lower()
    subjects = row.get("subjects", "").lower()

    # Check if any exclude keyword appears in either field
    if any(kw in bookshelves or kw in subjects for kw in exclude_keywords):
