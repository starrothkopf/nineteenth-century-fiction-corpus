...
def clean_and_filter(df, name):
    ...
    if 'place' in df.columns:
        ...
        df['nationality'] = 'British or Irish'  # add consistent nationality label
    elif 'nationality' in df.columns:
        df = df[df['nationality'].str.lower().str.contains('british|irish', na=False)]
        df['nationality'] = 'British or Irish'

    df = df[df['place'].str.lower().isin(british_irish_codes) if df['place'] in df.columns]
    df = df[df['nationality'].str.lower().isin(british_irish_codes) if df['place'] in df.columns]
