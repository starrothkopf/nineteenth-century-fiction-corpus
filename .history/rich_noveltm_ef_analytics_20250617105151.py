...
df['imprint'] = df['imprint'].apply(parse_list)
all_imprint = sum(df['imprint'], [])
imprint_counts = Counter([c for c in all_imprint]).most_common(20)
...
print("\n--- top imprints ---")
for imprint, count in lcc_counts:
    print(f"{lcc}: {count}")
